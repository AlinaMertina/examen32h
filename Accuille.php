...
class Accuille extends CI_Controller {
    public function __construct(){
        ...
        $this->load->model('ajoutbd_model','accuille');
        session_start();
     }
    ...
    public function Ajoutcategorie(){
        $this->accuille->ajoutcategorie($_POST['categorie']);
        redirect(base_url('accuille/liste/'));
    }
    public function listePourcentage(){
       
        $_SESSION['idpechangeprc']=$_GET['idproduit'];//indice de mon produit
        $tabe=array();
       
        $data=$this->accuille->getlisteproduitlien($_GET['prix'],$_GET['pourc'],$_SESSION['user'],$_GET['idproduit']);
        for($i=0;$i<count($data);$i++){
            $tabe['prix'][$i]=$data[$i]['prix'];
            $tabe['Nomimage'][$i]=$data[$i]['Nomimage'];
            $tabe['idp'][$i]=$data[$i]['idp'];
            ...
            //calcule 
                $value= (($tabe['prix'][$i]*100)/$_GET['prix'])-100;
            $tabe['dif'][$i]=$value;
        }
        $data=$this->accuille->get_categorie();
        for($i=0;$i<count($data);$i++){
        ...
        $this->load->view('header',$tabe);
        $this->load->view('resultapourcentage',$tabe);
        $this->load->view('footer');
    }
    public function AjoutpropositionPorcentage(){
        $this->ajout->ajoutproposition($_SESSION['user'],$_GET['iduser'],$_GET['idproduit'],$_SESSION['idpechangeprc']);
